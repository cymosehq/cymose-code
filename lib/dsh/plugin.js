...
export function apply(ctx, config) {
    ...
    ctx.tools.register(defineTool({
        ...
        output: textOut(),
        async execute(args) {
            const allowed = ["active", "done", "failed", "dead-end"];
            if (!allowed.includes(args.status)) {
                throw new Error(`status must be one of ${allowed.join(", ")}`);
            }
            const g = store(config);
            const id = requireId(g, args.id);
            const node = g.setStatus(id, args.status);
            return `"${node.title}" is now ${node.status}.`;
        },
    }));
    ctx.tools.register(defineTool({
        name: "cymose_summarize",
        description: "Write the summary children will inherit. You compose it here using this harness's model — nothing is sent to another service. Outcome is your verdict.",
        parameters: {
        ...
        async execute(args) {
            ...
            if (args.outcome === "failed")
                g.setStatus(id, "failed");
            else if (args.outcome === "done")
                g.setStatus(id, "done");
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args) {
            ...
            if (!left)
                throw new Error(`No node ${args.a}.`);
            if (!right)
                throw new Error(`No node ${args.b}.`);
