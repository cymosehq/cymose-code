...
const VOICE = {
    session: {
        ...
        emptyPrompt: "Cymose is loaded. There is no focused node. Call cymose_kind if this is a todo, steps, or answer graph, then cymose_branch — or cymose_load to restore a dump.",
    ...
    todo: {
        ...
        failedTag: " [blocked]",
        emptyPrompt: "Cymose todo graph is empty of focus. Call cymose_branch for an item, or cymose_load a dump.",
    ...
    steps: {
        ...
        failedTag: " [abandoned]",
        emptyPrompt: "Cymose steps graph has no focused step. Call cymose_branch for the first step, or cymose_load a dump.",
    ...
    answer: {
        ...
        failedTag: " [rejected]",
        emptyPrompt: "Cymose answer graph has no focused claim. Call cymose_branch to start one, or cymose_load a dump.",
        emptyTree: "No claims yet. Call cymose_branch to start the answer.",
        forked: "Forked rival claims (parent stays focused):",
    },
};
export function voiceFor(kind) {
...
function bodyFor(node, kind) {
    if (node.summary?.trim())
        return node.summary.trim();
    if (node.status === "failed" || node.status === "dead-end") {
        if (kind === "todo")
            return "(No note stored — still blocked. Do not treat it as open work.)";
        if (kind === "steps")
            return "(No note stored — still an abandoned step. Do not replay it blindly.)";
        if (kind === "answer")
            return "(No note stored — still a rejected line. Do not reuse it as settled.)";
        return "(No summary stored — still a failed path. Do not retry it blindly.)";
    }
    return "(No summary yet.)";
}
function failed(node, voice) {
 ...
 */
export function inheritText(current, ancestors, siblings = [], kind = "session") {
    ...
    if (ancestors.length > 0) {
        ...
        for (const node of ancestors) {
            ...
            if (node.promoted?.trim())
                lines.push(`Folded in: ${node.promoted.trim()}`);
            lines.push("");
        }
    }
...
export function mapForPrompt(focused, ancestors, siblings, kind = "session") {
    const voice = voiceFor(kind);
    if (!focused)
        return voice.emptyPrompt;
    return inheritText(focused, ancestors, siblings, kind);
}
export function treeListing(nodes, focusedId, kind = "session") {
    const voice = voiceFor(kind);
    if (nodes.length === 0)
        return `${voice.header}\n${voice.emptyTree}`;
