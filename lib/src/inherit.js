 ...
 */
export function inheritText(current, ancestors, siblings = []) {
    const lines = [
...
export function mapForPrompt(focused, ancestors, siblings) {
    if (!focused) {
        return [
...
export function treeListing(nodes, focusedId) {
    if (nodes.length === 0)
        return "The graph is empty. Call cymose_branch to start a node.";
