...
const STATUSES = ["active", "done", "failed", "dead-end"];
export const toolSpecs = [
    {
        name: "cymose_tree",
        description: "Show the Cymose graph: what is on the map, what failed, what is focused. Call this to see context.",
        inputSchema: { type: "object", properties: {} },
    },
    {
        name: "cymose_kind",
        description: "Set how to read this graph: session (what was tried), todo (open work), steps (procedure with forks), or answer (claims and evidence). Same tools; different map wording.",
        inputSchema: {
            type: "object",
            properties: {
                kind: {
    ...
    {
        name: "cymose_branch",
        description: "Start a new Cymose node. Children inherit ancestor summaries. Omit parent_id to fork from the focused node, or to create a root if the graph is empty.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    {
        name: "cymose_inherit",
        description: "Return ancestor summaries for the focused node (or id). Read this before repeating an approach that already failed.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    },
    {
        ...
        description: "Write the summary children will inherit. You compose it using this harness's model — nothing is sent to another service. Outcome is your verdict.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    {
        name: "cymose_explore",
        description: "Fork the focused node (or parent_id) into several sibling approaches. You invent the titles; each child inherits the same ancestors.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    {
        name: "cymose_pick",
        description: "Copy summaries from other nodes onto the target, labeled by origin. Use this when an exact wording or a failed-path note must travel, not a new synthesis.",
        inputSchema: {
            type: "object",
            properties: {
    ...
    {
        name: "cymose_dump",
        description: "Return the graph as JSON. Ask the harness to keep that JSON in the workspace if you want it after this process ends. This plugin does not touch the disk.",
        inputSchema: { type: "object", properties: {} },
    },
    {
        name: "cymose_load",
        description: "Replace the in-process graph with JSON previously shown by cymose_dump (or read by the harness from the workspace).",
        inputSchema: {
            type: "object",
            properties: {
...
export const MAP_INSTRUCTIONS = "Cymose is a map, not a coding agent. Call cymose_kind to choose session, todo, steps, or answer. Call cymose_tree or cymose_inherit before repeating work. Persist with cymose_dump; restore with cymose_load. The graph lives in this process.";
...
function requireId(store, id) {
    const resolved = id ?? store.load().focused_id;
    if (!resolved)
        throw new Error("Nothing is focused. Call cymose_branch or cymose_focus.");
    return resolved;
}
function titlesFrom(args) {
...
export function runTool(store, name, rawArgs = {}) {
    ...
    switch (name) {
        ...
        case "cymose_branch": {
            ...
            if (parentId)
                parent = parentId;
            else if (loaded.focused_id)
                parent = loaded.focused_id;
            else
                parent = null;
        ...
        case "cymose_inherit": {
            ...
            if (!id)
                return "Nothing is focused. Call cymose_branch or cymose_focus.";
        ...
        case "cymose_diff": {
            ...
            if (!left)
                throw new Error(`No node ${String(args.a)}.`);
            if (!right)
                throw new Error(`No node ${String(args.b)}.`);
